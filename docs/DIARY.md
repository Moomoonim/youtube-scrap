...
### 5) 사례집 작성 (같은 날 후속)
- 자동 생성 목록에서 **21건을 골라 원문을 직접 열고 수기 정리** → `docs/AITF_CASE_DOSSIER.md`
  (사례별: 출처·발화 주체·구현 내용·12칸 점수·원문 인용·프레임워크 해석·인용 주의)
- 🔴 <u>**채널 ≠ 발화 주체 오귀속 2건 교정**</u> — ① Davos 패널의 Copilot 13~14% 절감 발언은
  Infosys가 아니라 **Danske Bank CEO**의 것 ② "자동화→증강 이동" 발언은 Intel이 아니라
  **액센추어 CTO**(Intel 채널 팟캐스트 게스트). 자동 추출물은 채널명으로 표기되므로
  <u>사례 인용 전 원문에서 발화자를 확인하는 절차를 표준으로 삼는다</u>.
- 규칙 기반 코딩의 과소평가 사례 확인: Palantir 문서는 유지보수 티켓·에너지 소비·보증 상태 등
  **도메인 특수 데이터 어휘**를 사전이 못 잡아 데이터 축이 3점에 그침 → 사전 보강 후보.
- **2차 정독(같은 날)**: 짧은 영상은 전문, 긴 영상은 해당 구간을 통독해 사례집을 **구현 상세 중심**으로 재작성.
  발췌 인용 위주 → ①출발점 ②무엇을 넣었나 ③어떻게 굴렸나(절차·거버넌스·확산) ④결과 ⑤발화자가 인정한 한계 구조로 전환.
  <u>키워드 창(window) 추출만으로는 사례의 실행 구조가 안 보인다</u>는 것이 방법상 교훈 — 예: NEC는 발췌만 보면
  '대시보드 100개'뿐이지만 전문에는 애드온 1,600개 클린코어·에이전트로 컨설팅 공수 75%↓·8만 명 상시 사용이 들어 있다.
- 정독 중 **신규 사례 5건 발견**: Tampa General Hospital(패혈증 48h 사망률 68%↓·2년간 700명 추가 퇴원, Palantir 세션),
  Novo Nordisk(규제 제출문서 10주→10분), Southern California Edison(배전 부하 준실시간 최적화),
  Sysco(영업 통화계획·창고 경로), Unilever 베트남 쿠치 '다크 팩토리'(운영자 1명).

### 6) 흐름(프로세스) 분석 — `docs/AITF_CASE_FLOWS.md` (같은 날 3차)
- 사례를 정지 화면이 아니라 **시간축**으로 재구성. 귀납적으로 <u>전환 6단계(압력→계기→착수→구조화→확산→재정의)</u>를
  뽑고 논문의 framing/narrating/stretching에 대응시킴. 주사례 8건(NEC·PMI·Zapier·Danske·Unilever·Amazon Robotics·
  LG·Great Eastern)의 단계표 + mermaid 흐름도 + 큐브 궤적, 교차 사례 분석(경로 4유형·계기·구조화 장치·성과 번역), 명제 P1~P6.
- 🔴 <u>**핵심 발견 P1: 조직은 데이터를 갖춘 뒤 시작하지 않는다 — 다른 축을 올리다 데이터에 부딪혀 되돌아온다.**</u>
  큐브 위 이동이 단조 증가가 아니라 **backtracking을 포함**한다는 점에서 논문 Fig.2의 순차 이행 그림과 어긋난다.
- P3(톱다운은 원인이 아니라 초기 성과의 결과 — PMI), P5(성숙할수록 성과가 **분모 교체**로 표현 — Unilever FTE당 톤,
  LG 채택률 100%), P6(확산 제약은 기술이 아니라 제도 — 분기 실적·규제·인력 영향 평가).
- **기업×연도 패널 구현 후 기각**: `build_year_panel()` 추가(144셀·다년 채널 15개, `analysis/aitf_company_year.csv`).
  <u>추이가 조직 궤적이 아니라 **수집 커버리지 편차 + 콘텐츠 장르 혼합**을 반영</u>(Unilever 2024:10.3→2026:0.0은
  2026년 수집분이 브랜드 영상이기 때문). 담론 강도 시계열로는 유효하나 성숙도 궤적으로는 불가 — 커버리지 지표와
  장르 코딩이 선결 과제.

### 7) 내러티브 판본 — `docs/AITF_CASE_NARRATIVES.md` (같은 날 4차)
- 표·도식으로는 전환 논리가 전달되지 않는다는 지적에 따라 **읽히는 서사**로 재작성.
  기업별 <u>상황 → 계기 → 결정 → 실행 → 마찰 → 결과 → 남은 문제</u> 구성, 주사례 8건 + 짧은 사례 6건.
  자료가 끊긴 곳은 **[기록 공백]**, 필자 연결은 **[추정]**으로 명시.
- 서사로 재구성하니 드러난 것: 🔴 <u>**전환의 다음 단계는 실패가 아니라 성공이 만든다**</u> —
  PMI는 도입이 잘 되자 사용사례 300건이 쏟아져 관리 불능이 됐고, H&M은 챗봇이 잘 돌자 토큰 비용이 새 의제가 됐다.
- 성과 언어의 단계적 변화 확인: 초기=**시간**(21만 시간·12h→2h) → 중간=**공수·기간**(개발공수 -24%·출시기간 -40%)
  → 성숙=**분모 교체**(FTE당 톤·AI 스케줄 채택률 100%·문의 처리율 20%). <u>성과 언어 자체가 성숙도 지표일 수 있다.</u>
- 🔴 <u>가장 조용한 지점은 **사람**</u> — Unilever·NEC 서사에는 증강이 한 번도 등장하지 않는다.
  증강이 없는 것인지 말해지지 않는 것인지는 공개 담론으로 답할 수 없어 **인터뷰로 이월**.

### 8) 다음 단계
- 원 논문 설문(12문항)을 인터뷰 가이드에 삽입해 **담론 코딩 vs 조직 자기보고**를 삼각검증
- 도입 기업(adopter) 채널만 따로 떼어 벤더 담론과 셀 분포 비교
- 사례집의 수치는 K1(기업 공개 자료)과 교차 검증 — 특히 투자자 대상 발표(Unilever)는 공시 대조 가능
