...
### 1) 주요 키워드
**Vial(2019)**, DT 과정 모형 8개 구성요소, **사례 티어(A/B/C)**, AX 교량 3축, `map_vial.py`
...
- 산출: `analysis/vial_cases.csv`(태깅 전량) · `analysis/vial_evidence.csv`(구성요소별 근거 문장) ·
  `docs/VIAL_CASES.md`(티어별 사례 목록 + 근거 발췌)
...
### 4) 진도 / 결과
- **사례 613건** (A 42 · B 67 · C 504), 그중 **ax_core 575건**(94%)
- 티어 A 예: <u>INSEAD *Making AI Transformation Work: Avoiding the Mistakes from Digital
  ...
  Unilever 투자자 이벤트 4건, ServiceNow K26 키노트, Telefónica CMD 2025, SAP·Snowflake·
  Databricks 키노트, Zapier RevOps/Shadow AI</u>
...
- <u>B6 장벽(관성·저항) 370건이 B5 구조 변화(조직구조 151·문화 61)보다 많음</u> — 문제 제기는 하되
  조직 재설계 해법은 덜 말하는 비대칭
- 🔴 <u>X3 동적역량 903건 · X2 윤리 728건 — Avenue 1·2 모두 담론에서는 이미 두껍다.
  Vial(2019)의 연구 어젠다를 **AX 맥락에서 실증할 표본이 확보**된 셈</u>

### 6) 이슈 및 다음 단계
- 규칙 기반의 한계(문맥·반어 미포착)는 코드북 v2와 동일 — <u>티어 A/B(109건)는 **수기 검증 대상
  후보 목록**으로 쓰는 것이 정직</u>
- 다음: 티어 A 수기 코딩(구성요소별 실제 성립 여부) → 신뢰도(κ) 산출 → Vial 과정 모형의
  **AX 확장 명제**(어느 블록이 AI 맥락에서 변형되는가) 도출
- `map_vial.py` 는 `build_catalog.py` 와 같이 **수동 실행**(일일 워크플로 미포함, 회당 ~6분)
