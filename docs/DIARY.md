...
## 260804 AITF 매핑: 코퍼스를 AI Transformation Framework 3축에 코딩

### 1) 주요 키워드
**AI Transformation Framework**(Holmström & Magnusson, 2026), **자동화·증강·데이터 풍부성**,
큐브 8상태, integrated intelligence, 사례 후보 추출

### 2) 작업 내용
- 논문의 3차원 × 4항목(Level·Scope·Impact·Future, Table 1) 구조를 그대로 코딩 스킴으로 옮긴
  `map_aitf.py` 추가 — 축 핵심어 밀도(Level) + 범위·수치·전략 표지의 **근접 동시출현**(±400자)
  으로 Scope/Impact/Future를 매기고, 축별 0~16점 → 논문 절단점(≥9=high)으로 큐브 좌표 판정
- 산출: `analysis/aitf_scored.csv`(영상 2,777건) · `analysis/aitf_company.csv`(채널 80개) ·
  `docs/AITF_CASES.md`(8개 상태별 사례 후보 + 수치 근거 발췌)

### 3) 관찰
- 🔴 <u>**논문 절단점을 담론 자료에 그대로 적용하면 거의 전부 low로 떨어진다**(자동화 2.2%,
  데이터 1.3%, 증강 0.4%만 high) — 공개 담론은 수준·범위·영향·전략 네 항목을 동시에 갖춰
  말하는 일이 드물고, **부분적 주장**의 형태를 띠기 때문</u>. 이것 자체가 관찰이며,
  설문(자기보고)과 담론은 같은 척도로 취급될 수 없다는 방법론적 근거가 된다.
- <u>축별 강조는 **자동화(1.50) > 데이터 풍부성(1.20) > 증강(0.77)**</u> — 논문이 목표점으로
  제시한 integrated intelligence(3축 동시 high)와 대비하면 담론은 자동화로 치우쳐 있고
  **증강 서사가 가장 얇다**. AT(알고리즘 전환) 프레임과도 방향이 맞는 소견.
- 사례 후보로 쓸 만한 수치 주장 확보: Unilever(배송거리 15%↓·트럭 활용률 10%↑),
  Great Eastern Life/Oracle(비용 30%↓), H&M 챗봇(전환율 25%↑), Zapier 회계(월말 결산 25%↓),
  NEC(대시보드 100개·전사 데이터 표준화), Infosys(Copilot 회의시간 13~14%↓) 등 — **전부 원문 대조 확인**.

### 4) 한계
- 담론 ≠ 성숙도. 자사 채널 발화는 마케팅 목적이 섞이며, 셀 배치는 **발화의 강조점**이지
  조직 상태가 아니다. 상대 절단점은 코퍼스 의존적이라 표본이 바뀌면 배치도 바뀐다.
- 논문이 이름 붙인 꼭짓점은 `manual isolated data scarcity`·`integrated intelligence` 둘뿐이라
  나머지 6개 셀은 **본 매핑의 기술적 명명**임을 명시.

### 5) 다음 단계
- 원 논문 설문(12문항)을 인터뷰 가이드에 삽입해 **담론 코딩 vs 조직 자기보고**를 삼각검증
- 도입 기업(adopter) 채널만 따로 떼어 벤더 담론과 셀 분포 비교
