...
- `build_verhoef_dossier.py` 추가 — 배정된 사례를 **카드 단위로 펼침**(판정 근거 마커·주장 문장
  최대 5개·성과 수치·언급 기업·Table 1 좌표). 단계별 4개 문서, 총 **1,142장**.
- `docs/VERHOEF_S3_PROFILES.md` — S3 상위 사례를 **원문 스크립트로 직접 검증**한 서술형 프로파일 13건
  + 오탐 사례 분석.

### 3) 결과 (ax_core 2,777건 기준)
| 단계 | 건수 | 비율 |
|---|---:|---:|
| S1 digitization | 29 | 1.0% |
| S2 digitalization | 1,158 | 41.7% |
| S3 digital transformation | 77 | 2.8% |
| unassigned(단계 언어 없음) | 1,513 | 54.5% |

- 🔴 <u>**S2가 S3의 15배**</u> — 이 코퍼스의 AX 담론은 압도적으로 **기존 프로세스의
...
- Table 1 패턴은 대체로 관측됨: S3 표본에서 민첩성·네트워킹·성장전략·디지털 KPI 출현율이
  S1/S2보다 뚜렷이 높다.
- S1이 희소한 것(29건)은 2020년대 코퍼스로서 자연스럽다 — 아날로그→디지털은 지나온 단계.
- 🔴 <u>원문 검증 결과 **S3에 실제 도달한 사례는 '과금·역할 로직'을 건드린다**</u> —
  EY(노력 기반 과금 파기)·Wistia(좌석 해체)·Snowflake(구독→소비)·YC(결과 기반 과금).
  Table 1의 네 번째 축(지표)이 사실상 **선행 지표**로 작동한다.
- Telenor×Cognite(2021)는 "데이터 가용화 → 조직 현대화 → 제로터치 운영"이라는 **자생적 3단계론**을
  제시하는데, Verhoef와 구조는 같되 <u>세 번째 칸이 BM이 아니라 **무인화**</u>다 — AT 축과 맞닿는 지점.
...
### 5) 이슈·한계
- 규칙 기반이라 문맥·반어·부정 미포착. 사전을 3차에 걸쳐 조였다:
  ① '가치 사슬/창출' IR 상투어 → 변화 서술이 붙을 때만 (S3 261→133건)
  ② `marketplace` 단독 → AWS 마켓플레이스 조달 오탐 제거 (133→77건)
  ③ `스캔` 단독 → 초음파·CT 등 의료 영상 오탐 제거 (S1 52→29건)
- 🔴 <u>카드에 **⚠️ 단일 마커 지배** 경고를 넣었다</u> — 한 단어가 단계 점수의 70% 이상이면 표시.
  남은 오탐(비-디지털 BM, 포트폴리오 M&A 언어)을 사람이 걸러낼 수 있게 하는 장치.
- 발화 주체는 vendor/media 2분까지만 자동 판별 — 사례 인용 전 원문 확인 필수.
- 자막 오인식 사례 확인: Unilever IR에서 **EBITDA → "AIDA"** 전사로 `AI` 오탐.
