# 수집 콘텐츠 검토 보고서 (v6.2, 2026-07-28 갱신)

> **5,155건 전체 코퍼스** 기준 분석. 심층 정독 ~1,010건(키워드 128 + 채널 대부분), 대규모 백필 잔여분은 인벤토리 상태.
> 자동분류: **AX 1,345 / DX 90 / AT 8 / 미분류 3,712** (채널 208개, 한국어 4,168 / 영어 987). 연도: 2020:39 / 2021:87 / 2022:161 / 2023:242 / 2024:375 / 2025:1,032 / 2026:3,219(62%).
> **증분 현황**: v6.1(3,865)→현재(5,155)는 **전량 확증적** — 신규 substantive 채널 없음(대형 채널 백필 위주). 지도 변경 없음, v5~v6 수렴 구조 유지.
> **🔧 수집 시기 확장(2026-07-28)**: 시계열 분석을 위해 **2020년부터 전 구간**을 목표로 과거 백필을 가속(채널당 8→20편/회, 총 150→300/회, 목록 1200→3000, config.py). CHANNEL_SINCE는 이미 20200101(floor=2020). 향후 회차부터 2020~2022 구간이 더 빠르게 채워질 예정 — 다만 시계열 주장은 백필이 고르게 찰 때까지 여전히 유보(§9).
...
8. **자동차 산업 AI 태도 4분화**(v4 확장) — 침묵(Nissan·Volvo) → **서사적 침묵(Mercedes)** → 완곡 내재화(BMW) → 정체성화(Tesla). "AI를 말하지 않는 방식"에도 전략적 스펙트럼 존재. **v6.2**: 🔑 **현대차가 정체성화(Tesla) 극점에 합류** — 정의선 "우리는 이제 자동차 회사가 아니다"(AI 서밋), 피지컬 AI에 **51조 투자**(엔비디아·구글·웨이모 제휴). "디지털 AI는 틀려도 되나 피지컬 AI는 절대 틀리면 안 된다"(물리AI 신뢰성 프레임, Zoox/Wayve와 수렴).
