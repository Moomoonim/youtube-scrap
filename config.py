...
# ※ 2020년부터 전 구간을 목표로 함(시계열 분석용 t0). 아래 상향된 회차 상한으로
#   과거(2020~2022) 구간까지 백필이 더 빠르게 도달하도록 [2026-07-28 조정].
CHANNEL_SINCE = "20200101"

# 실행 1회당 채널 하나에서 새로 수집할 최대 영상 수
# [2026-07-28] 8→20: 채널당 더 깊이 파고들어 과거(2020~) 구간에 빨리 도달
MAX_PER_CHANNEL_PER_RUN = 20

# 실행 1회당 전체 채널에서 수집할 총 상한 (실행 시간 관리용)
# 하루 6회 실행 → 하루 최대 = 이 값 × 6. [2026-07-28] 150→300
# (실제 상한은 CHANNEL_TIME_BUDGET_MIN=90분 예산이 먼저 걸어 안전)
MAX_CHANNEL_TOTAL_PER_RUN = 300

# 실행 1회당 채널 하나에서 시도(성공+실패 포함)할 최대 영상 수
# — 자막 없는 영상이 많은 채널이 실행 시간을 독식하는 것을 방지
# [2026-07-28] 15→40: 과거 영상은 자막 없는 경우가 많아, 자막 있는 영상까지
#   도달하려면 더 많은 시도가 필요(백필 깊이 확보)
MAX_ATTEMPTS_PER_CHANNEL_PER_RUN = 40
